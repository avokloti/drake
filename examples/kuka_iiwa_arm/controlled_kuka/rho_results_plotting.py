...
for i in np.arange(num_solvers):
    objectives_succ.append(objectives[i, successes[i,:]])
    constraints_succ.append(constraints[i, successes[i,:]])
    feasibility_succ.append(feasibility[i, successes[i,:]])
    times_succ.append(times[i, successes[i,:]])
...
plt.subplot(1, 2, 1)
ax = plt.gca()
ax.boxplot(np.transpose(times_succ))
ax.set_xticklabels(solver_names, rotation='vertical')
plt.ylim([0, 5])
plt.title('Runtime')
plt.xlabel('Solvers')
...
plt.subplot(1, 2, 2)
ax = plt.gca()
ax.boxplot(np.transpose(objectives_succ))
ax.set_xticklabels(solver_names, rotation='vertical')
...
# plot constraint satisfaction error for solvers, as boxplot

plt.figure()

plt.subplot(1, 2, 1)
ax = plt.gca()
ax.boxplot(np.transpose(feasibility_succ))
ax.set_xticklabels(solver_names, rotation='vertical')
plt.title('Feasibility Error')
plt.xlabel('Solvers')
plt.ylabel('Inf-norm Error')

plt.subplot(1, 2, 2)
ax = plt.gca()
ax.boxplot(np.transpose(constraints_succ))
ax.set_xticklabels(solver_names, rotation='vertical')
