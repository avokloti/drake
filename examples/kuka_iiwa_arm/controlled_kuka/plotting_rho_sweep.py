...
def checkObjectives(Q, Qf, R):
    ...
    for index in np.arange(num_trials * num_problems):
        ...
        states = np.loadtxt(dir + solvers[0] + "_x_" + str(index) + ".txt")
        sum_of_squares = 0;
        for inp in range(num_inputs):
            sum_of_squares = sum_of_squares + np.sum(R[inp, inp] * inputs[:, inp+1]**2)
        
        for st in np.arange(int(num_states/2), num_states):
            sum_of_squares = sum_of_squares + np.sum(Q[st, st] * states[:, st+1]**2)
        
        diff = np.abs(objectives[index] - sum_of_squares)
        #if (diff > 1e-5):
            #print(str(index) + ": error in objective calculation! Difference is " + str(diff))
            #error_count = error_count + 1
        if np.isnan(objectives[index]) and (results[index] == 'SolutionFound'):
            print(str(index) + ": objective is NAN.")
...
dir = "/Users/ira/Documents/drake/examples/kuka_iiwa_arm/controlled_kuka/output/rho_sweep_basic/"
...
checkObjectives(0.01 * np.eye(14), 0, 0.001 * np.eye(7))
...
plt.tight_layout()
#plt.show()
plt.savefig('kuka_arm_rho_sweep.png')
