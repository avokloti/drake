...
dir = "/Users/ira/Documents/drake/examples/complete_robobee/output/rho_sweep_v1/"
...
plt.subplot(1, 3, 2)
im = plt.imshow(np.transpose(iterations_mean), norm=Normalize(vmin=0, vmax=1000), cmap='viridis', interpolation='nearest')
plt.colorbar(im)
...
plt.subplot(1, 3, 3)
im = plt.imshow(np.transpose(objectives_mean), norm=Normalize(vmin=6.5, vmax=8), cmap='viridis', interpolation='nearest')
plt.colorbar(im)
