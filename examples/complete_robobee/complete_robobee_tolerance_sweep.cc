    ...
    namespace examples {
        namespace quadrotor {
            namespace {
                ...
                /* WRITE HEADER FILE */
                int writeHeaderFile(std::string filename, int trial, Eigen::Ref<Eigen::MatrixXd> traj_x, Eigen::Ref<Eigen::MatrixXd> traj_u, int total_iterations, double time, double feas_tolerance, double opt_tolerance, std::string solve_result) {
                ...
                Eigen::VectorXd solveOPT(solvers::MathematicalProgramSolverInterface* solver, std::string solver_name, double feas_tolerance, double opt_tolerance, int trial, Eigen::Ref<Eigen::VectorXd> initial_traj) {
                    ...
                    if (solver_name.find("ipopt") != std::string::npos) {
                        traj_opt.SetSolverOption(solvers::IpoptSolver::id(), "tol", opt_tolerance);
                        traj_opt.SetSolverOption(solvers::IpoptSolver::id(), "acceptable_tol", opt_tolerance);
                    ...
                    writeHeaderFile(solver_name, trial, xtraj, utraj, -1, elapsed_time.count(), feas_tolerance, opt_tolerance, solutionResultToString(result));
                ...
                Eigen::VectorXd solveADMM(systems::trajectory_optimization::admm_solver::AdmmSolverBase* solver, std::string solver_name, double feas_tolerance, double opt_tolerance, int trial, Eigen::Ref<Eigen::VectorXd> initial_traj) {
                    ...
                    writeStateToFile(solver_name, trial, xtraj);
                    writeInputToFile(solver_name, trial, utraj);
                    writeHeaderFile(solver_name, trial, xtraj, utraj, total_iterations, elapsed_time.count(), feas_tolerance, opt_tolerance, solve_result);
                    ...
                    //for (int trial = 8; trial < 9; trial++) {
                        ...
                        for (int i = 0; i < int(opt_tolerances.size()); i++) {
                            for (int j = 0; j < int(feas_tolerances.size()); j++) {
                                ...
                                double feas_tolerance = feas_tolerances.at(j);
                                ...
                                Eigen::VectorXd admm_traj =  solveADMM(solver_admm, "admm", feas_tolerance, opt_tolerance, index, zero_traj);
                                //Eigen::VectorXd snopt_traj = solveOPT(solver_snopt, "snopt", feas_tolerance, opt_tolerance, index, zero_traj);
                                //Eigen::VectorXd ipopt_traj = solveOPT(solver_ipopt, "ipopt", feas_tolerance, opt_tolerance, index, zero_traj);
