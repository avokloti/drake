...
def getConstraints2(z):
    g = np.asarray([[z[1, 0]**2 - z[0, 0]],
                    [z[0, 0] - (z[1, 0]**2)/4 - 2],
                    [z[0, 0]**8 + z[1, 0]**8 - 0.5**8]])
    G = np.matrix([[-1, 2 * z[1, 0]],
                   [1, -z[1, 0]/2],
                   [8 * z[0, 0]**7, 8 * z[1, 0]**7]])
    return [g, G]
...
def objectiveUpdate(z, Q, q, lambda1, rho1):
    ...
    first_update_norms.append(np.linalg.norm(matrix))
    first_update_diff_norms.append(np.linalg.norm(inv @ (rho1 * z) - z))
    return inv @ (rho1 * z - lambda1 - q)

def constraintsUpdate(w, G, h, lambda1, lambda2, rho1, rho2):
    matrix = rho1 * np.identity(2) + rho2 * G.T * G
    inv = np.linalg.inv(matrix)
    second_update_norms.append(np.linalg.norm(matrix))
    second_update_diff_norms.append(np.linalg.norm(inv @ (rho1 * w) - w))
    return inv @ (rho1 * w + lambda1 + G.T * (rho2 * h - lambda2))
...
first_update_norms = []
second_update_norms = []
first_update_diff_norms = []
second_update_diff_norms = []
...
for i in np.arange(n):
    if (complementarity == 0):
        [g, G] = getConstraints0(z)
    elif (complementarity == 1):
        [g, G] = getConstraints1(z)
    elif (complementarity == 2):
        [g, G] = getConstraints2(z)
...
print("y = " + str(z[1, 0]))
print("===============================")
#print(first_update_norms)
#print(second_update_norms)
#print(first_update_diff_norms)
#print(second_update_diff_norms)
...
plt.figure()
plt.subplot(4, 1, 1)
plt.plot(first_update_norms)
plt.title("First update matrix norms")
plt.subplot(4, 1, 2)
plt.plot(second_update_norms)
plt.title("Second update matrix norms")
plt.subplot(4, 1, 3)
plt.plot(first_update_diff_norms)
plt.title("First update diff norms")
plt.subplot(4, 1, 4)
plt.plot(second_update_diff_norms)
plt.title("Second update diff norms")
plt.tight_layout()
