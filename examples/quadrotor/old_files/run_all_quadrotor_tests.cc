...
#include "drake/systems/trajectory_optimization/ali_solver_res.h"
    ...
    namespace examples {
        namespace quadrotor {
            namespace {
                ...
                std::string output_folder = "/Users/ira/Documents/drake/examples/quadrotor/output/scaling/";
                ...
                /* WRITE HEADER FILE */
                int writeHeaderFile(std::string filename, int trial, Eigen::Ref<Eigen::MatrixXd> traj_x, Eigen::Ref<Eigen::MatrixXd> traj_u, double tolerance, double time, std::string solve_result) {
                ...
                void initializeObstaclesAndBounds(std::string problem_type, int trial) {
                    if (problem_type == "obstacles" || problem_type == "obstacles_warm_start" || problem_type == "obstacles_interp") {
                        num_obstacles = 2;
                    }  else if (problem_type == "bugtrap") {
                        num_obstacles = 3;
                    }
                ...
                solvers::SolutionResult solveOPT(solvers::MathematicalProgramSolverInterface* solver, std::string solver_name, double tolerance, int trial, std::string problem_type) {
                    
                    std::cout << " ------------------------- Solving " << problem_type << " with " << solver_name << " -------------------------" << std::endl;
                    
                    initializeObstaclesAndBounds(problem_type, trial);
                    
                    systems::trajectory_optimization::MidpointTranscription traj_opt(quadrotor, *quadrotor_context_ptr, N, T/N, T/N);
                    //systems::trajectory_optimization::DirectCollocation traj_opt(quadrotor, *quadrotor_context_ptr, N, T/N, T/N);
                    ...
                    if (problem_type == "obstacles" || problem_type == "obstacles_warm_start") {
                        for (int i = 0; i < num_obstacles; i++) {
                    ...
                    } else if (problem_type == "obstacles_interp") {
                    ...
                    writeHeaderFile(solver_name, trial, xtraj, utraj, 1e-4, elapsed_time.count(), solutionResultToString(result));
                    return result;
                }
                
                
                /* SOLVE ADMM WITH OBSTACLES */
                void solveADMM(systems::trajectory_optimization::admm_solver::AdmmSolverBase* solver, std::string solver_name, double tolerance, int trial, std::string problem_type) {
                    ...
                    if (solver_name == "admm_pen") {
                        ...
                        solver->setRho2(10);
                        solver->setRho3(10);
                    } else if (solver_name == "admm_sc") {
                        solver->setRho1(0.001);
                        solver->setRho2(0.05);
                        solver->setRho3(0.5);
                    }
                    ...
                    // solve
                    std::string result = solver->solve(y);
                    ...
                    writeStateToFile(solver_name, trial, xtraj);
                    writeInputToFile(solver_name, trial, utraj);
                    writeHeaderFile(solver_name, trial, xtraj, utraj, tolerance, elapsed_time.count(), solve_result);
                }
                ...
                int do_main(int argc, char* argv[]) {
                    ...
                    // make solver
                    systems::trajectory_optimization::admm_solver::AdmmSolverBase* solver_w = new systems::trajectory_optimization::admm_solver::AdmmSolverWeightedV2(quadrotor, x0, xf, T, N, 1000);
                    
                    //systems::trajectory_optimization::admm_solver::AdmmSolverBase* solver_al = new systems::trajectory_optimization::admm_solver::AdmmSolverAL(quadrotor, x0, xf, T, N, 1000);
                    
                    systems::trajectory_optimization::admm_solver::AdmmSolverBase* solver_sc = new systems::trajectory_optimization::admm_solver::AdmmSolverScaled(quadrotor, x0, xf, T, N, 1000);
                    
                    systems::trajectory_optimization::admm_solver::AdmmSolverBase* solver_al_ineq = new systems::trajectory_optimization::admm_solver::AdmmSolverALIneq(quadrotor, x0, xf, T, N, 300);
                    
                    systems::trajectory_optimization::admm_solver::AdmmSolverBase* solver_ali_res = new systems::trajectory_optimization::admm_solver::ALISolver(quadrotor, x0, xf, T, N, 300);
                    ...
                    vector<std::string> test_problems {"simple", "simple_warm_start", "obstacles", "obstacles_warm_start", "obstacles_interp"};
                    //vector<std::string> test_problems {"obstacles_warm_start"};
                    //vector<std::string> test_problems {"obstacles"};
                    
                    //for (int i = 0; i < int(test_problems.size()); i++) {
                    for (int i = 3; i < 4; i++) {
                        solveADMM(solver_sc, "admm_sc", tolerance, i, test_problems.at(i));
                        //solveADMM(solver_al, "admm_al", tolerance, i, test_problems.at(i));
                        //solveADMM(solver_ali_res, "ali_res", tolerance, i, test_problems.at(i));
                        //solveADMM(solver_al_ineq, "admm_al_ineq", tolerance, i, test_problems.at(i));
                        //solveOPT(ipopt_solver, "ipopt", tolerance, i, test_problems.at(i));
                        //solveOPT(snopt_solver, "snopt", tolerance, i, test_problems.at(i));
                    }
