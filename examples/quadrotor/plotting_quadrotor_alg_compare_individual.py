...
def drawFinal2D():
    ...
    plt.plot(admm_traj[:, 1], admm_traj[:, 2], c=colors[0], label='ADMM')
    plt.plot(snopt_traj[:, 1], snopt_traj[:, 2], c=colors[1], label='SNOPT')
    plt.plot(ipopt_traj[:, 1], ipopt_traj[:, 2], c=colors[2], label='IPOPT')
...
def drawFinal3D():
    ...
    line = ax.plot(admm_traj[:, 1], admm_traj[:, 2], admm_traj[:, 3], c=colors[0], label='ADMM')
    ax.plot(snopt_traj[:, 1], snopt_traj[:, 2], snopt_traj[:, 3], c=colors[1], label='SNOPT')
    ax.plot(ipopt_traj[:, 1], ipopt_traj[:, 2], ipopt_traj[:, 3], c=colors[2], label='IPOPT')
    # Setting the axes properties
...
admm_traj = states_output[0]
snopt_traj = states_output[1]
ipopt_traj = states_output[2]
...
# directory
traj_filename = dir + "admm_traj_" + str(index) + ".txt"
print(traj_filename)
