...
obj_admm = objectives[0:20]
obj_admm = objectives[solve_results == 'SolutionFound']
...
for s in np.arange(len(solvers)):
    for i in np.arange(10):
        ind = num_trials * s + i
        ax.plot(states_output[ind][:, 1], states_output[ind][:, 2], states_output[ind][:, 3], color = colors[s], label = solvers[s], marker='o', markersize=2, linewidth=0.5)
        ax.scatter(x0s[ind][0], x0s[ind][1], x0s[ind][2], s=1, color='black')
        ax.scatter(xfs[ind][0], xfs[ind][1], xfs[ind][2], s=1, color='black')
