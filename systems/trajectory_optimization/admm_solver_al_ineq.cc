    ...
    namespace systems {
        namespace trajectory_optimization {
            namespace admm_solver {
                ...
                AdmmSolverALIneq::AdmmSolverALIneq(systems::System<double>* par_system, Eigen::VectorXd par_x0, Eigen::VectorXd par_xf, double par_T, int par_N, int par_max_iter) : AdmmSolverBase(par_system, par_x0, par_xf, par_T, par_N, par_max_iter) {
                ...
                AdmmSolverALIneq::AdmmSolverALIneq(systems::System<double>* par_system) : AdmmSolverBase(par_system) {
                    //initial_rho1 = 500;
                    //initial_rho2 = 200;
                    //initial_rho3 = 200;
                ...
                std::string AdmmSolverALIneq::solve(Eigen::Ref<Eigen::VectorXd> y) {
                    ...
                    //while (i < max_iter && (i < 1 || feasibilityVector.lpNorm<Eigen::Infinity>() > tol_feasibility || constraintVector.lpNorm<Eigen::Infinity>() > tol_constraints)) {
                        ...
                        for (int iii = 0; iii < int(single_constraints_list.size()); iii++) {
                            ...
                            for (int ii = 0; ii < N; ii++) {
                                ...
                                for (int iv = 0; iv < cf.length; iv++) {
                                    ...
                                    //if ((cf.flag == INEQUALITY) & (single_g[iv] < 0)) {
                                        //lambda3[index] = std::max(lambda3[index]/2, 0.0);
                                        //std::cout << "Decreasing lambda3" << std::endl;
                                    //}
                                    ...
                                    //if ((single_g[iv] <= 0) & (single_g[iv] >= -lambda3[index]/rho3)) {
                                    //    std::cout << "Negative term has some effect! " << ii << " -- " << cf.constraint_name << std::endl;
                                    //}
                        ...
                        //objective = (y.transpose() * R * y) + (y.transpose() * b);
                        ...
                        //if (i <= 1) {
                        //    std::cout << "feasibilityVector:\n" << feasibilityVector << std::endl;
                        //}
                ...
                Eigen::VectorXd AdmmSolverALIneq::proximalUpdateConstraints(Eigen::Ref<Eigen::VectorXd> x, Eigen::Ref<Eigen::VectorXd> lambda1, Eigen::Ref<Eigen::VectorXd> lambda2, Eigen::Ref<Eigen::VectorXd> lambda3, Eigen::Ref<Eigen::SparseMatrix<double> > M, Eigen::Ref<Eigen::VectorXd> c, Eigen::Ref<Eigen::SparseMatrix<double> > G, Eigen::Ref<Eigen::VectorXd> h, double rho1, double rho2, double rho3) {
