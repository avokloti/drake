...
namespace drake {
    namespace systems {
        namespace trajectory_optimization {
            ...
            MidpointTranscriptionConstraint::MidpointTranscriptionConstraint(const System<double>& system, const Context<double>& context, int num_states, int num_inputs) : Constraint(num_states + num_alpha * obstacle_radii_x.size(), 1 + (2 * num_states) + (2 * num_inputs), Eigen::VectorXd::Zero(num_states), Eigen::VectorXd::Zero(num_states)),
            ...
            void MidpointTranscriptionConstraint::DoEval(
                ...
                // fill the first num_states with below:
                *y = x1 - (x0 + h * g);
                
                // fill the last num_alpha * num_obstacles with obstacle constraints
            ...
            continuous_state_(system_->AllocateTimeDerivatives()) {
                ...
                auto constraint = std::make_shared<MidpointTranscriptionConstraint>(*system, context);
                auto obstacle_constraint = std::make_shared<InterpolatedObstacleConstraint>(*system, context);
            ...
            void InterpolatedObstacleConstraint::DoEval(const Eigen::Ref<const AutoDiffVecXd>& x, AutoDiffVecXd* y) const {
                ...
                *y = math::initializeAutoDiff(Eigen::VectorXd::Ones(obstacle_radii_x_.size() * num_alpha_));
                
                std::cout << "obstacle interp constraint y size: " << y->size() << ", x size: " << x.size() << std::endl;
            }
