    ...
    namespace systems {
        namespace trajectory_optimization {
            namespace admm_solver {
                ...
                ALISolver::ALISolver(systems::System<double>* par_system, Eigen::VectorXd par_x0, Eigen::VectorXd par_xf, double par_T, int par_N, int par_max_iter) : AdmmSolverBase(par_system, par_x0, par_xf, par_T, par_N, par_max_iter) {
                    ...
                    //rho2_increase_rate = 1.05; // increases by a constant factor with every iteration
                    //rho3_increase_rate = 1.05;
                }
                
                ALISolver::ALISolver(systems::System<double>* par_system) : AdmmSolverBase(par_system) {
                    initial_rho1 = 50;
                    //initial_rho2 = 200;
                    //initial_rho3 = 200;
                    
                    //rho1_decrease_rate = 1.05;
                    //rho2_increase_rate = 1.0; // increases by a constant factor with every iteration
                    //rho3_increase_rate = 1.0;
                }
                
                /* ---------------------------------------------- SOLVE METHOD ---------------------------------------------- */
                
                std::string ALISolver::solve(Eigen::Ref<Eigen::VectorXd> y) {
                    ...
                    std::cout << "Norm of R is: " << R.norm() << ", largest element is: " << R.maxCoeff() << std::endl;
                    std::cout << "Norm of b is: " << b.norm() << ", largest element is: " << b.maxCoeff() << std::endl;
                    
                    /*
                    ofstream R_stream;
                    R_stream.open("/Users/ira/Documents/drake/examples/kuka_iiwa_arm/controlled_kuka/output/R.txt");
                    if (!R_stream.is_open()) {
                    ...
                    while (i < max_iter && (not_feasible || not_converged)) {
                        ...
                        for (int iii = 0; iii < int(single_constraints_list.size()); iii++) {
                            ...
                            for (int ii = 0; ii < N; ii++) {
                                ...
                                // zero out inequalities that are not violated and update weights
                                for (int iv = 0; iv < cf.length; iv++) {
                                    int index = ii * num_constraints + running_constraint_counter + iv;
                                    
                                    if ((cf.flag == INEQUALITY) & (single_g[iv] <= -lambda3[index]/rho3)) {
                                        single_g[iv] = 0;
                                        single_dg_x.block(iv, 0, 1, num_states) = 0 * single_dg_x.block(iv, 0, 1, num_states);
                                        single_dg_u.block(iv, 0, 1, num_inputs) = 0 * single_dg_u.block(iv, 0, 1, num_inputs);
                                        lambda3[index] = lambda3[index]/1.05;
                                    }
                            ...
                            for (int ii = 0; ii < N-1; ii++) {
                                ...
                                if (cf.flag == INEQUALITY) {
                                    for (int iiii = 0; iiii < cf.length; iiii++) {
                                        int index = ii * num_constraints + running_constraint_counter + iiii;
                                        if (single_g[iiii] <= -lambda3[index]/rho3) {
                                            single_g[iiii] = 0;
                        ...
                        /*
                         ...
                         std::cout << "Norm of G = " << G.norm() << std::endl;
                         } else {
                         G.setZero();
                         std::cout << "G should just be 0... norm = " << G.norm() << std::endl;
                         } */
                        G.setFromTriplets(tripletsG.begin(), tripletsG.end());
                        ...
                        lambda3 = lambda3.cwiseMax(0);
                        
                        //std::cout << "||lambda1|| = " << lambda1.lpNorm<2>() << ", ||lambda2|| = " << lambda2.lpNorm<2>() << ", ||lambda3|| = " << lambda3.lpNorm<2>() << ", ||G y - h|| = " << (G * y - h).lpNorm<2>() << std::endl;
                        //std::cout << (lambda3/rho3).transpose() << std::endl;
                        
                        // increase rho2 and rho3
                        objective = y.dot(R * y) + y.dot(b);
                        ...
                        //rho2 = min(rho3 * rho3_increase_rate, rho_max);
                        
                        double primal_res = rho1 * (x - y).lpNorm<2>() + rho2 * feasibilityVector.lpNorm<2>() + rho3 * constraintVector.lpNorm<2>();
                        double dual_res = rho1 * (y - y_prev).lpNorm<2>();
                        
                        std::cout << "-- dual res: " << dual_res << " -- primal res: " << primal_res << " -- rho1: " << rho1 << " -- rho2: " << rho2 << " -- rho3: " << rho3 << std::endl;
                        
                        // what do we want to do?
                        // if the trajectory is moving towards feasibility and optimality, don't change anything
                        // if the trajectory has stopped moving towards feasibility:
                        //      if it is under tolerance, decrease rho1 to move towards optimality
                        //      if it is not under tolerance, increase
                        // if the trajectory is
                        
                        
                        if (20 * primal_res < dual_res) {
                            rho1 = min(rho1 * 1.02, rho_max);
                        } else if (20 * dual_res < primal_res) {
                            rho1 = max(rho1 / 1.1, rho1_min);
                        } else {
                            // nothing happens? or altenatively:
                            rho2 = min(rho2 * rho2_increase_rate, rho_max);
                        ...
                        // print / compute info
                        if (i % 1 == 0) {
                            output_stream << "Iteration " << i << " -- objective cost: " << objective << " -- feasibility (inf-norm): " << feasibilityNorm << " -- constraint satisfaction (inf-norm): " << constraintNorm << "-- full objective: " << full_objective << "rho1: " << rho1 << ", rho2: " << rho2 << ", rho3: " << rho3 << "\n";
                        }
                ...
                Eigen::VectorXd ALISolver::proximalUpdateConstraints(Eigen::Ref<Eigen::VectorXd> x, Eigen::Ref<Eigen::VectorXd> lambda1, Eigen::Ref<Eigen::VectorXd> lambda2, Eigen::Ref<Eigen::VectorXd> lambda3, Eigen::Ref<Eigen::SparseMatrix<double> > M, Eigen::Ref<Eigen::VectorXd> c, Eigen::Ref<Eigen::SparseMatrix<double> > G, Eigen::Ref<Eigen::VectorXd> h, double rho1, double rho2, double rho3) {
